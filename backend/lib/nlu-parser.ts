...
export async function parseInput(
  ...
  // Build enhanced prompt with extracted URL info
...
2. Map to the correct domain based on the DATABASE SCHEMA and DOMAIN SCHEMAS above
3. Extract ALL fields required by the domain schema
4. For URLs: Extract company from hostname, role from path segments (especially after /careers/, /jobs/, /positions/)
5. Normalize units (convert to standard: ml, cups, oz, liters for water; hours for sleep; kg for weight)
6. Normalize habit names (e.g., "quitting smoking" -> "quit smoking")
7. Extract numbers, dates, times, currencies from text
8. If information is incomplete, set incomplete: true and ask for clarification
